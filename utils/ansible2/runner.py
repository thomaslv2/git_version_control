...
def get_default_options():
    options = Options(
        ...
        listhosts=False,
        syntax=False,
        timeout=60,  #ssh的timeout时间
        connection='ssh', #代表ssh的连接
        module_path='', #module的路径
        forks=10, #指定并发
        remote_user='root',  #指定运行用户
...
class AdHocRunner(object):
    ...
    def __init__(self, inventory, options=None):
        if options:
            self.options = options
        self.inventory = inventory  #动态生成的机器
...
class CommandRunner(AdHocRunner):
    ...
    def execute(self, cmd, pattern="all", module=None):
