...
Net = window.Net || {
	...
	ajax:function(options){
		...
		var defaults = {
		   ...
		   go:null,
		   boxClose:false,
		   btn:null,  //不可用的按钮，也就是那个按钮加上disabled
		   icon:$("#iconWait"),
		   callback:null,
		   ...
		   beforeSend:function(){
			   if(opts.btn != null){
				   opts.btn.attr('disabled','disabled'); //disabled 当前图标不可点
			   }
			   if(opts.icon != null){
				   opts.icon.addClass("fa fa-spinner fa-spin") //loading
			   }
		   },
		   complete:function(){
			   if(opts.btn != null){
